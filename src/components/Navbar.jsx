...
function Navbar() {
  ...
  // Fixed variants for proper mobile behavior
  const menuVariants = {
    hidden: { x: "-100%" }, // Changed from 100% to -100% to slide from left
    visible: { 
      x: 0, 
      transition: { 
        type: "spring", 
  ...
  const linkVariants = {
    hidden: { opacity: 0, x: -30 }, // Changed from x: 30 to x: -30
    visible: (i) => ({
      opacity: 1,
      x: 0,
      transition: { delay: i * 0.1, duration: 0.4 },
  ...
  return (
    <>
      {/* Main Navbar */}
      <div className="flex justify-between h-20 items-center px-6 text-white fixed w-full top-0 z-40 shadow-lg bg-black/40 backdrop-blur-md">
        {/* Logo */}
        <h1 className="text-3xl font-extrabold font-kalam bg-gradient-to-r from-gray-500 via-white to-gray-800 bg-clip-text text-transparent tracking-wider cursor-pointer">
          RA<br />ZAK.
        </h1>
        ...
        {/* Mobile Toggle */}
        <div onClick={handleNav} className="block md:hidden cursor-pointer z-50 relative">
          {nav ? (
            <AiOutlineClose
              size={26}
              className="hover:text-gray-400 transition-colors"
        ...
        {nav && (
          <>
            ...
            <motion.div
              ...
              exit="exit"
              variants={menuVariants}
              className="fixed top-0 left-0 w-[75%] sm:w-[60%] h-full backdrop-blur-lg bg-zinc-900/95 border-r border-gray-700 flex flex-col z-40 md:hidden"
            >
              {/* Menu Header */}
              <div className="flex justify-between items-center p-6 border-b border-gray-700">
                <h1 className="text-2xl font-extrabold font-kalam bg-gradient-to-r from-gray-500 via-white to-gray-800 bg-clip-text text-transparent">
                  RA<br />ZAK.
                </h1>
                <AiOutlineClose
                  size={26}
                ...
                {navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      ...
                      smooth={true}
                      duration={800}
                      className="block w-full cursor-pointer hover:text-gray-400 transition-colors"
                    >
                      {link.label}
                    </Link>
                  </motion.li>
